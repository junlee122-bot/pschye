...
class FrontierVillageScene extends Phaser.Scene {
  ...
  private createTarget(model: VillageModel) {
    ...
    const sprite = fitImageHeight(this.add.image(0, 0, `target-${model.sceneId}`).setOrigin(0.5, 1), targetHeight);
  ...
  update(time: number, delta: number) {
    ...
    if (time - this.lastPositionBroadcast > 120) {
      ...
      const landmark = this.nearestLandmark();
      if (landmark !== this.lastLandmark || moving) {
        this.lastLandmark = landmark;
        this.game.events.emit(POSITION_EVENT, { x: this.player.x, y: this.player.y, landmark } satisfies VillagePosition);
      }
    }
...
export function VillageAdventure({ profile, onChoose, onAdvance, onExit, onMove }: VillageAdventureProps) {
  ...
  }), [interaction, profile.world.village.playerX, profile.world.village.playerY, scene.id, scene.speakerName]);
  ...
  }, [dialogueOpen, onAdvance, selectedChoice]);
  ...
  return (
    <main className={`village-adventure ambience-${interaction.ambience}`} style={{ backgroundImage: `linear-gradient(180deg, rgba(4, 8, 5, .34), rgba(3, 5, 4, .76)), url(${villageMapAssets[interaction.ambience]})` }}>
      ...
      <header className="village-hud-top">
        <button onClick={onExit}><ArrowLeft size={16} /> 타이틀</button>
      ...
      <div className="village-mobile-controls" aria-label="모바일 이동 조작">
        <button onPointerDown={() => move(0, -1)} aria-label="위로 이동"><ArrowUp /></button>
        <button onPointerDown={() => move(-1, 0)} aria-label="왼쪽으로 이동"><ArrowLeft /></button>
        <button onPointerDown={() => move(0, 1)} aria-label="아래로 이동"><ArrowDown /></button>
        <button onPointerDown={() => move(1, 0)} aria-label="오른쪽으로 이동"><ArrowRight /></button>
      </div>
